...
void TakeDataAnalyzer(Int_t RunN){
 ...
 if(NoCSharing){
 	string FileOutName = "ProcessedDataNoCSharing.root";
 }else {
 ...
 //*************************Checking in which event there could be charge sharing******************
 bool EventWithCSharing[16][200000];
 for(int j=0; j <200000;++j){
   for(int k=0; k< 16;++k){
      EventWithCSharing[k][j] = false;
   }
 }
 for(Int_t i =0; i< iNTlenght; ++i){
    Digis->GetEntry(i);
    if((abs(col - PulsedCol) <= 1) && (abs(row - PulsedRow) <= 1) && (row != PulsedRow) && (col !=PulsedCol)){
       EventWithCSharing[roc][EvN] = true; 
     }
 }
 ...
 for(Int_t i =0; i< iNTlenght; ++i){
    ...
    if(OldEvN != EvN ||( i == iNTlenght -1) ){
      for(Int_t j=0; j < 16; ++j){ 
        ...
        Multiplicity->Fill(EvN-1, j,NModHits ,NROCHits ,NRows ,NCols ,MeanPh, MeanPhPuls);
      }
      ModMult.Clean();
      OldEvN = EvN;
    }
    ...
    bool SelPixel = 0;
    if(row ==PulsedRow && col ==PulsedCol && (!EventWithCSharing[roc][EvN] || !NoCSharing)) SelPixel = 1;
    if(row>-1&&col>-1)ModMult.Add(roc, row, col, ph, SelPixel);
 }
