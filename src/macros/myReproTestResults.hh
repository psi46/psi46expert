#ifndef MYREPROTESTRESULTS
#define MYREPROTESTRESULTS
...
class MyReproTestResults {
public :

  MyReproTestResults(TTree *tree=0);
  ~MyReproTestResults();
  ...
  const char *PrintNumber(Double_t number, Int_t nDigis);
...
private:
  ...
  int nModules, nA, nB, nC, allGrades[2000], allOverallGrades[2000], allFinalGrades[2000], allShortGrades[2000], allDates[2000], allAllTestsDone[2000], gradeOption;
  ...
  double tempVoltageRMS;
  int nPHDC;

  double *noiseSum, *noise2Sum;
  double *uthrSum, *uthr2Sum;
  double *tthrSum, *tthr2Sum;
  double *gainSum, *gain2Sum;
  double *pedSum, *ped2Sum;
  int *trimBitSum, *trimBit2Sum;
  double *par1Sum, *par12Sum;
  ...
  TH1F *hTrimmedThresholdRMS, *hUntrimmedThresholdRMS, *hTrimBitsRMS;
  TH1F *hIV150, *hIVSlope;
  TH1F *hNoiseRMS;
  TH1F *hGainRMS, *hPedestalRMS, *hPar1RMS;
  TH1F *hIA, *hID, *hVA, *hVD;
  TH1F *hDead, *hMask, *hBumps, *hAddress, *hTrimBitDefects;
  TH1F *hvdig[16], *hvana[16], *hvsf[16], *hvhlddel[16], *hvtrim[16], *hvthrcomp[16], *hvoffsetop[16], *hvibias_ph[16], *hibias_dac[16], *hvibias_roc[16], *hcaldel[16];
  TH1F *hTempVoltage[16];
